...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!token) {
      return NextResponse.json({ success: false, error: "Authentication required to pull packets." }, { status: 401 });
    }

    const decoded = verifyToken(token) as { userId: string };

    if (!quizId) {
      return NextResponse.json({ success: false, error: "Target identifier missing." }, { status: 400 });
    }
    ...
    if (!targetQuiz) {
      return NextResponse.json({ success: false, error: "Target set not found." }, { status: 404 });
    }
    ...
    const structuralQuiz = await prisma.$transaction(async (tx) => {
    ...
    return NextResponse.json({ success: true, downloadCount: structuralQuiz.downloadCount });
  } catch (error) {
    ...
    console.error("INCREMENT_DOWNLOAD_ERROR:", error);
    return NextResponse.json({ success: false, error: "Database tracking index crash." }, { status: 500 });
  }
}
